...
// https://chromium.googlesource.com/chromium/src/media/+/master/video/h264_parser.h
const (
	NALU_UNSPECIFIED     = 0
...
const (
	...
	PROFILE_IDC_CONSTRAINED_BASELINE = PROFILE_IDC_BASELINE
	PROFILE_IDC_MAIN                 = 77
	PROFILE_ID_SCALABLE_BASELINE     = 83
	PROFILE_ID_SCALABLE_HIGH         = 86
	PROFILE_IDC_HIGH                 = 100
	PROFILE_ID_HIGH10                = 110
	PROFILE_IDS_MULTIVIEW_HIGH       = 118
	PROFILE_ID_HIGH422               = 122
	PROFILE_ID_STEREO_HIGH           = 128
...
// a tighter bound could be calculated if someone cares about a few bytes
const MAX_MBPAIR_SIZE = 256 * 1024

func ExtractRBSP(src []byte, smallPadding bool) (H2645NAL, int) {
	padding := 0
	i := 0
	length := len(src)

	if !smallPadding {
		padding = MAX_MBPAIR_SIZE
	}
	...
	if i >= length-1 && smallPadding {
		return H2645NAL{Raw: src[:length], Rbsp: src[:length]}, length
	} else if i > length {
		i = length
	}
	rbspBuffer := make([]byte, length+padding)
...
nsc:
	return H2645NAL{Raw: src[:si], Rbsp: dst[:di]}, si
}

// returns -1 if EOF, -2 invalid
func getNALSize(nalLengthSize int, buf []byte, bufIndex int) int {
	i := 0
	nalsize := 0
	if bufIndex >= len(buf)-nalLengthSize {
		log.Logf(log.ERROR, "NAL end of buffer")
		return -1
	}
	for i = 0; i < nalLengthSize; i++ {
		nalsize = (nalsize << 8) | int(buf[bufIndex])
		bufIndex++
	}
	if nalsize <= 0 || nalsize > len(buf)-bufIndex {
		log.Logf(log.ERROR, "Invalid NAL unit size (%d > %d).", nalsize, len(buf)-bufIndex)
		return -2
	}
	return nalsize
}

type H2645NAL struct {
	Raw      []byte
	Rbsp     []byte
	SizeBits int
...
func getBitLength(nal H2645NAL, skipTrailingZeros bool) int {
	size := len(nal.Rbsp)
	for skipTrailingZeros && size > 0 && nal.Rbsp[size-1] == 0 {
		size--
	}
	...
	if v != 0 {
		size -= trailingZeros8(v) + 1
	}
	return size
}

func h264ParseNALHeader(nal H2645NAL) (H2645NAL, int) {
	if len(nal.Rbsp) == 0 {
		return nal, -1
	}
	h := nal.Rbsp[0]
...
func h265ParseNALHeader(nal H2645NAL) (H2645NAL, int) {
	if len(nal.Rbsp) < 2 {
		return nal, -1
	}
	h := nal.Rbsp[0]
	...
	nal.Type = int((h >> 1) & 0x3f)
	nuhLayerID := ((h & 0x1) << 5) | ((h1 >> 3) & 0x1f)
	nal.TemporalID = int(h1&0x7) - 1
...
func SplitNALUs(src []byte, isAVC bool, nalLengthSize int, codecType av.CodecType, smallPadding bool) (nalus []H2645NAL) {
	length := len(src)
	nextAVCOffset := length
	if isAVC {
		nextAVCOffset = 0
	}
	bufOffset := 0
	...
	for length >= 4 {
		...
		if bufOffset == nextAVCOffset {
			i := 0
			extractLength = getNALSize(nalLengthSize, src[bufOffset:], i)
			if extractLength < 0 {
				return
			}
			bufOffset += nalLengthSize
			length -= nalLengthSize

			nextAVCOffset = bufOffset + extractLength
		} else {
			if bufOffset > nextAVCOffset {
				log.Log(log.WARN, "Exceeded next NALFF position, re-syncing.")
			}
			for src[bufOffset] != 0 || src[bufOffset+1] != 0 || src[bufOffset+2] != 1 {
				bufOffset++
				length--
				if length < 4 {
					if len(nalus) == 0 {
						log.Log(log.ERROR, "No start code is found.")
					}
					// No more start codes: we discarded some irrelevant
					// bytes at the end of the packet.
					return
				} else if bufOffset >= nextAVCOffset-3 {
					break
				}
			}
			...
			bufOffset += 3
			length -= 3
			extractLength = nextAVCOffset - bufOffset
			...
			if bufOffset >= nextAVCOffset {
				...
				offset := nextAVCOffset - bufOffset
				bufOffset += offset
				length -= offset
				continue
			}
		}
		nal, consumed := ExtractRBSP(src[bufOffset:bufOffset+extractLength], smallPadding)
		if consumed < 0 {
			return
		}

		if isAVC && extractLength != consumed && extractLength > 0 {
			log.Log(log.DEBUG, "NALFF: Consumed only %d bytes instead of %d", consumed, extractLength)
		}

		/* see commit 3566042a0 */
		if consumed < length-3 &&
			src[bufOffset+consumed] == 0 && src[bufOffset+consumed+1] == 0 &&
			src[bufOffset+consumed+2] == 1 && src[bufOffset+consumed+3] == 0xe0 {
			skipTrailingZeros = false
		}

		nal.SizeBits = getBitLength(nal, skipTrailingZeros)
		pret := 0
		if codecType == av.H265 {
			nal, pret = h265ParseNALHeader(nal)
		} else {
			nal, pret = h264ParseNALHeader(nal)
		}
		if pret > 0 && len(nal.Rbsp) > 0 {
			nalus = append(nalus, nal)
		} else {
			log.Log(log.ERROR, "Invalid NAL unit %d, skipping.", nal.Type)
		}
...
func GuessNALUType(b []byte) (nalus [][]byte, typ int) {
	if len(b) < 4 {
...
func ParseSPS(data []byte) (self SPSInfo, err error) {
	...
	// NOTE reversed bit order
	if self.ConstraintSetFlags, err = r.ReadBits(8); err != nil {
		return
	}
	if self.ConstraintSetFlags&0x3 != 0 {
		err = errors.New("h264: reserved_zero_2bits invalid")
		return
	}
	...
	if self.SeqParameterSeqID, err = r.ReadExponentialGolombCode(); err != nil {
		return
	}
...
func NewCodecDataFromAVCDecoderConfRecord(record []byte) (cd CodecData, err error) {
	cd, err = newCodecDataFromAVCDecoderConfRecord(record)
	if err == nil {
		return
	}
	if len(record) < 4 {
	...
	// Some buggy MP4 use AnnexB format
	if record[0] == 0 && record[1] == 0 &&
		(record[2] == 1 || (record[2] == 0 && record[3] == 1)) {
		log.Log(log.WARN, "AVCDecoderConfRecord seems to be buggy annex B")
		nals := SplitNALUs(record, false, 4, av.H264, true)
		if len(nals) < 2 {
			err = ErrDecconfInvalid
			return
		}
		cd, err = NewCodecDataFromSPSAndPPS(nals[0].Raw, nals[1].Raw)
		cd.RecordInfo.AnnexBPacket = true
		return
	}
	return
}
...
func NewCodecDataFromSPSAndPPS(sps, pps []byte) (self CodecData, err error) {
	if len(sps) < 4 {
		err = fmt.Errorf("h264parser: invalid sps")
		return
	}
	recordinfo := AVCDecoderConfRecord{}
	...
	nal, _ := ExtractRBSP(sps, true)
	if self.SPSInfo, err = ParseSPS(nal.Rbsp); err != nil {
		log.Log(log.WARN, "rtp: invalid h264 sps")
		return
	}
	return
}
...
func readFFLongUint(r *bits.GolombBitReader) (v uint, err error) {
	var b uint
...
func writeFFLongUint(w *GolombBitWriter, v uint) error {
	for {
		if v >= 0xff {
...
func ParseSEIMessageFromNALU(packet []byte) (sei SEIMessage, err error) {
	...
	if sei.Type, err = readFFLongUint(r); err != nil {
		return
	}
	if sei.PayloadSize, err = readFFLongUint(r); err != nil {
		return
	}
	...
	sei.Payload = sei.Payload[:sei.PayloadSize]

	return
}

func RBSPToNALData(rbsp []byte) []byte {
	buf := &bytes.Buffer{}
	buf.Grow(len(rbsp) + len(rbsp)/3)
	count := 0
	for i := 0; i < len(rbsp); i++ {
		if (count == 2) && (rbsp[i]&0xfc == 0) {
			_ = buf.WriteByte(0x03)
			count = 0
		}
		_ = buf.WriteByte(rbsp[i])
		if rbsp[i] == 0x00 {
			count++
		} else {
			count = 0
...
func (self SEIMessage) Marshal() (H2645NAL, error) {
	h := H2645NAL{Type: NALU_SEI}
	buf := &bytes.Buffer{}
	w := NewGolombBitWriter(buf)
	...
	if err := w.WriteBits(NALU_SEI, 5); err != nil {
		return h, err
	}

	if err := writeFFLongUint(w, self.Type); err != nil {
		return h, err
	}
	if err := writeFFLongUint(w, self.PayloadSize); err != nil {
		return h, err
	}
	for _, v := range self.Payload[:self.PayloadSize] {
		if err := w.WriteBits(uint(v), 8); err != nil {
			return h, err
		}
	}
	rbspStopOneBit := uint(1)
	if err := w.WriteBit(rbspStopOneBit); err != nil {
		return h, err
	}
	...
	h.Rbsp = buf.Bytes()
	h.Raw = RBSPToNALData(h.Rbsp)
	h.SizeBits = getBitLength(h, true)
	return h, nil
}
